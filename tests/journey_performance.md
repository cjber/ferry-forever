...
Run from the repo root. `$SPF_HARNESS` in these records is the external harness these runs used; its stubs are now `tests/ui_stubs.lua`; its own scenarios had gone stale and were not kept. The archive command only reads git state.
