...
from wowmock import (
	BUILD, FONTS, NORMAL, WHITE, TrackerBlock, TrackerModule, TooltipLine, Ui,
	backdrop, colored, draw_overlay, map_art, map_overlays, minimap_art, objective_tracker,
	rgba255, scene, tooltip, tooltip_backdrop, world_map_frame,
)
...
def projection(ui, point, map_id):
	rows = [r for r in ui.table("UiMapAssignment").values()
		if int(r["UiMapID"]) == map_id and int(r["MapID"]) == point["map"]
		and r["WMODoodadPlacementID"] == "0"]
	if not rows:
		return None
	...
	return tuple(float(r[f"UiMin_{i}"]) + n * (float(r[f"UiMax_{i}"]) - float(r[f"UiMin_{i}"]))
		for i, n in enumerate((x, y)))


def dock_name(dock_id):
	d = data()["docks"][str(dock_id)]
	near = sorted((math.hypot(t["x"] - d["x"], t["y"] - d["y"]), int(key), t)
		for key, t in data()["taxis"].items() if t["map"] == d["map"])
...
def dock_title(dock_id):
	...
	near = [d for key, d in data()["docks"].items() if key in boats and d["map"] == dock["map"]
		and (d["x"] - dock["x"]) ** 2 + (d["y"] - dock["y"]) ** 2 < 800 ** 2]
	# Map.lua's DockPierName uses all nearby boat piers, independent of zoom or discovered routes.
	landing = pier_direction(sum(d["y"] for d in near) / len(near) - dock["y"],
		dock["x"] - sum(d["x"] for d in near) / len(near)) if len(near) > 1 else "dock"
...
def segment(canvas, a, b, color, dashed=False, alpha=1):
	...
	if length == 0:
		return
	intervals = [(d / length, min(d + 6, length) / length) for d in range(0, math.ceil(length), 11)] if dashed else [(0, 1)]
...
def map_docks(ui, map_id, width, height):
	...
	kinds = {int(stop["dock"]): route["kind"] for route in data()["routes"].values()
	...
	for key, dock in sorted(data()["docks"].items(), key=lambda item: int(item[0])):
		...
		if p and all(0 <= n <= 1 for n in p):
			docks.append((int(key), max(0.015, min(0.985, p[0])) * width,
				max(0.015, min(0.985, p[1])) * height))
	...
	return [(sum(d[1] for d in group) / len(group), sum(d[2] for d in group) / len(group),
		[d[0] for d in group], kinds[group[0][0]]) for group in groups.values()]
...
def map_canvas(ui, map_id=947, alpha=1, hover=False):
	...
	def curve(a, b, control, color, fade=False):
		...
		for step in range(1, 13):
			t = step / 12
			p = tuple(((1 - t) ** 2 * a[i] + 2 * (1 - t) * t * control[i] + t * t * b[i]) * size
		...
		for index, (a, b) in enumerate(zip(points, points[1:])):
			if a["map"] == b["map"] and not a.get("jump"):
				...
				for end, neighbor in ((a, before), (b, points[index + 2] if index + 2 < len(points) else points[1])):
					...
					distance = min(((1 if delta > 0 else 0) - origin) / delta
						for origin, delta in ((p[0], ex), (p[1], ey)) if delta)
					if distance > 0:
	...
	if not hover:
		for name in (("darkshore",) if map_id == 1439 else ("walk", "silithus") if map_id == 1414 else ("walk", "wetlands")):
	...
	if not hover:
		...
		if gp and 0 <= gp[0] <= mw and 0 <= gp[1] <= mh:
			...
			canvas.draw(art, mx + gp[0] - art.width * 0.4, my + gp[1] - art.height * 0.4, art.width * 0.8, art.height * 0.8)
...
def tracker_canvas(ui, seconds=0, boats=False, settling=False):
	if boats:
		module = TrackerModule("Boats", [TrackerBlock(dock_title(8), [
...
def compass_canvas(ui, facing=math.pi - 0.2, distance=780):
	...
	for index in range(24):
		...
		if index % 6 == 0:
			canvas.text(180 + x - 10, 12, ("N", "W", "S", "E")[index // 6], FONTS["GameFontNormalSmall"], (*NORMAL, alpha), justify="CENTER", width=20)
	...
	for angle, name, size, alpha, bend in (
		...
		if bend:
			...
			canvas.text(x - 40, 36 + h / 2 + 2, f"{distance} yd", FONTS["GameFontHighlightSmall"], justify="CENTER", width=80)
	return canvas
...
def render_minimap(ui):
	...
	project = lambda p: (size / 2 + (START["y"] - p["y"]) * size / (2 * radius), size / 2 - (p["x"] - START["x"]) * size / (2 * radius))
...
def render_demo(ui):
	...
	palette.putpalette(palette.getpalette()[:720] + [channel for color in reserved + [(0, 0, 0)] * 10 for channel in color])
	indexed = [frame.quantize(palette=palette, dither=Image.Dither.NONE) for frame in frames]
	buffer = io.BytesIO()
	indexed[0].save(buffer, format="GIF", save_all=True, append_images=indexed[1:], duration=100, loop=0, optimize=True, disposal=1)
...
def compare_references(refs, tooltip_ref=None):
	...
	comparison_sheet("tracker", [("Owner 21 (2x)", enlarged(reference),
		"Current source: totals in section header (2x)", enlarged(mock))])
	...
	comparison_sheet("world-map", [("Owner 14", reference, "Current overview curve / Wetlands goal", world),
		("Owner: Auberdine (3x)", enlarged(reference.crop((115, 125, 220, 205)), 3),
		"Mock: Auberdine (3x)", enlarged(world.crop((115, 125, 220, 205)), 3))])
	for name, number, map_id, box in (
		...
		comparison_sheet(name, [(f"Owner {number}: matched parchment crop (2x)", enlarged(reference),
			"Current route / selected destination (2x)", enlarged(mock))])
	hover = map_image(947, hover=True)
	comparison_sheet("docks", [("Owner 14: ferry beside Menethil (3x)",
		enlarged(Image.open(refs / "14.png").crop((590, 230, 674, 297)), 3),
		"Hover mock: same ferry, destination glow (3x)", enlarged(hover.crop((590, 230, 674, 297)), 3)),
		("Owner 14: Auberdine (3x)", enlarged(Image.open(refs / "14.png").crop((130, 135, 175, 175)), 3),
		"Hover mock: additive ferry highlight (3x)", enlarged(hover.crop((130, 135, 175, 175)), 3))])
	...
	comparison_sheet("minimap", [("Owner 19: Auberdine (2x)", enlarged(reference),
		"Mock: same tiles/frame; selected start and route (2x)", enlarged(mock)),
		("Owner 18: dashed route (3x)", enlarged(Image.open(refs / "18.png").crop((64, 34, 135, 156)), 3),
		"Mock: route and native waypoint (3x)", enlarged(render_minimap(art).image.crop((192, 280, 291, 354)), 3))])
	comparison_sheet("compass", [("Owner 20: previous compass (3x)",
		enlarged(Image.open(refs / "20.png").crop((0, 24, 313, 81)), 3),
		"Current source: 360x60, soft border, stock fonts (2x)", enlarged(compass_canvas(ui).image))])
	if tooltip_ref:
		# The supplied captures contain no dock tooltip. Use the skill's real stock GameTooltip capture.
		tip = tooltip(ui, [TooltipLine("Boats"), TooltipLine("Darkshore, south pier"),
			TooltipLine("to Wetlands", NORMAL, "arrives 3:23 · leaves 4:23")])
		ground = backdrop(ui, tip.width, tip.height)
		ground.paste(tip, 0, 0)
		comparison_sheet("tooltip", [("Real stock GameTooltip: SkillUp be11638 (2x)",
			enlarged(Image.open(tooltip_ref).crop((0, 0, 346, 50))),
			"Same stock widget, dock content from Map.lua (2x)", enlarged(ground.image))])
	if (OUT / "demo.gif").exists():
		with Image.open(OUT / "demo.gif") as demo:
			...
			comparison_sheet("demo", [(f"Frame {a}", samples[i], f"Frame {b}", samples[i + 1])
				for i, (a, b) in zip((0, 2, 4), ((0, 6), (26, 34), (48, 70)))])
...
def main():
	...
	for name in args.scenes:
		try:
			...
			print(f"{path.relative_to(ROOT)}  {len(content):,} bytes  sha256:{hashlib.sha256(content).hexdigest()}", flush=True)
